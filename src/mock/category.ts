...
registerMock('get', '/category/list', () => {
  return ok(listWithCount())
})

registerMock('post', '/category', (config) => {
  const body = config.data as CategoryRequest
...
registerMock('put', /^\/category\/.+$/, (config) => {
...
registerMock('delete', /^\/category\/.+$/, (config) => {
  const id = (config.url as string).split('/').pop()!
